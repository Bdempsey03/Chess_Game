    ...
    public void movePieceOnSquare(BoardSquare start, BoardSquare end, Move move) {
        ...
        if (start.pieceOnSquare != null) {
            if (start.pieceOnSquare.getColor() == Game.getPlayerTurn()) {
                ...
                ArrayList<Move> moves = piece.validMove(theBoard, start);//enclose this in "removedCheckMoves" once working
...
//                System.out.println("WKING - BKING" + KingModel.whiteKing + " - " + KingModel.blackKing);
    ...
    public BoardSquare matchIndexToBoard(String index, BoardSquare[][] theBoard){
        for (int i = 0; i < 64; i++) {
    ...
    public ArrayList<Move> removedCheckMoves(ArrayList<Move> oldMoves, BoardSquare[][] theBoard) {
        ...
        int b;
        boolean whiteTurn = Game.getPlayerTurn() == 'w';

        BoardSquare[][] tempBoard = new BoardSquare[8][8];
        ...
        for (int i = 0; i < oldMoves.size(); i++) {
            if (!oldMoves.get(i).getEnd().equals("null")) {
                ...
                BoardSquare theEnd = matchIndexToBoard(oldMoves.get(i).getEnd(), theBoard);
        ...
        int k = 0;
        while(k < oldMoves.size()) {
            if (oldMoves.get(k).toString().equals("null-null"))
                oldMoves.remove(k);
            else
                k++;
        }
        System.out.println(oldMoves.size() + "~~~~~~~~~~");
            return oldMoves;
    }
}
